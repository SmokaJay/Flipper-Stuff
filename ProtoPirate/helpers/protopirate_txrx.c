...
bool protopirate_hopper_update(ProtoPirateApp* app) {
    ...
    // Every time a full frequency sweep completes, also step to the next
    // modulation preset (AM/FM registries), so the caller never has to pick
    // frequency or modulation manually to find a signal.
    const size_t preset_count = preset_swept ? subghz_setting_get_preset_count(app->setting) : 0;
    if(preset_swept && preset_count > 0) {
        app->txrx->hopper_idx_preset = (app->txrx->hopper_idx_preset + 1) % preset_count;

        const char* preset_name =
            subghz_setting_get_preset_name(app->setting, app->txrx->hopper_idx_preset);
        uint8_t* preset_data =
            subghz_setting_get_preset_data(app->setting, app->txrx->hopper_idx_preset);
        size_t preset_data_size =
            subghz_setting_get_preset_data_size(app->setting, app->txrx->hopper_idx_preset);
