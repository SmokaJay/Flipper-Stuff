...
- **Auto-scan modulation.** Previously, `Hopping: ON` only stepped through
  ...
  preset stayed fixed at whatever was picked manually in `Modulation:`.
  Now, every time a full frequency sweep completes, the hopper also
  advances to the next modulation preset before continuing, so turning
...
(`ENABLE_EMULATE_FEATURE` gated, per `application.fam`).
